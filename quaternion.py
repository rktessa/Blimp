...
class Quaternion:
    ...
    def __str__(self):
        return "Valore output: {}".format(self.q)
    ...
    def to_angle_axis(self):
        ...
        if self[0] == 1 and self[1] == 0 and self[2] == 0 and self[3] == 0:
            return 0, 1, 0, 0
        deg = (np.arccos(self[0]) * 2)*180/np.pi
        imaginary_factor = np.sin(rad / 2)
        if abs(imaginary_factor) < 1e-8:
            return 0, 1, 0, 0
        x = (self._q[1] / imaginary_factor) *180/np.pi
        y = (self._q[2] / imaginary_factor) *180/np.pi
        z = (self._q[3] / imaginary_factor) *180/np.pi
        return deg, x, y, z
