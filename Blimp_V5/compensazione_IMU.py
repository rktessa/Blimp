...
f = open("C:\Volume_D\Programming\Blimp_git\Blimp\Blimp_V5\log_IMU_motion.txt", "r")
Lines = f.readlines()
...
mad = Madgwick(sampleperiod = 1/12, quaternion=q0, beta=0.0041)
...
for i in range(len(accelerations)-1):
    ...
    acc_abs = np.transpose(J(roll, pitch, yaw))@ (np.transpose(accelerations[i,:])+np.transpose(gyro_term)) - np.array([[0, 0, 9.96]]).T
