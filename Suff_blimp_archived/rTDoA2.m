...
stringData = split(str);

            for i=1:6
                ts(i,1) = bin2dec(stringData(i));
                ts(i,2) = bin2dec(stringData(i+6));
                ts(i,3) = bin2dec(stringData(i+12));
                ts(i,4) = bin2dec(stringData(i+18));
            end
...
%Drift tag
dt_new2 = (toa_rx(:,2)-toa_rx(:,1))./(toa_tx(:,2)-toa_tx(:,1))
...
[x_t, dt_new] = rTDoApippo(times(65,:), dt_new2)
